#!/usr/bin/env python3
"""
測試: Copilot SDK → ACP Proxy → Gemini CLI

執行方式:
    python tests/test_copilot_sdk_via_proxy.py

前置條件:
    - pip install github-copilot-sdk
    - 安裝 Gemini CLI (gemini)
    - 安裝本套件 (pip install -e .)

架構:
    Copilot SDK → copilot-acp-proxy (我們的) → gemini --experimental-acp

這個測試驗證:
    1. Copilot SDK 能夠連接到我們的 Proxy
    2. Proxy 能夠轉發請求到 Gemini CLI
...
async def main():
    print("=" * 60)
    print("測試: Copilot SDK → ACP Proxy → Gemini CLI")
    print("=" * 60)
    ...
    if RESULTS["proxy_check"]["available"]:
        ...
        print("  安裝方式: pip install -e .")
        return False

    # Check backend (Gemini)
    backend = "gemini"
    print(f"\n[Backend 檢查: {backend}]")
    ...
    # Test: Copilot SDK → ACP Proxy → Gemini
    print("\n[TEST] Copilot SDK → ACP Proxy → Gemini")
    ...
    try:
        ...
        session = await asyncio.wait_for(
            client.create_session({"model": "gemini-2.0-flash"}),
            timeout=30.0
        )
        print("  Session 建立成功 ✅")
        ...
        t2 = time.time()
        await session.send({"prompt": "Say 'Hello from Gemini via Proxy!' in one short sentence."})
    ...
    if all_pass:
        print("🎉 測試通過!")
        print("\n結論:")
        print("  Copilot SDK 可以透過 ACP Proxy 連接到 Gemini CLI!")
        print("  架構: Copilot SDK → copilot-acp-proxy → gemini --experimental-acp")
    else:
        print("⚠️ 測試失敗")
