#!/usr/bin/env python3
"""
測試: Copilot SDK → ACP Proxy → claude-code-acp

執行方式:
    uv run python tests/test_copilot_sdk_via_proxy_claude.py
...
架構:
    Copilot SDK → copilot-acp-proxy (我們的) → claude-code-acp

這個測試驗證:
    1. Copilot SDK 能夠連接到我們的 Proxy
    2. Proxy 能夠轉發請求到 claude-code-acp
...
async def main():
    print("=" * 60)
    print("測試: Copilot SDK → ACP Proxy → claude-code-acp")
    print("=" * 60)
    ...
    if os.environ.get("ANTHROPIC_API_KEY"):
        ...
        print("  ✅ 在 Claude Code 環境中運行 (使用內部認證)")
    else:
        print("  ⚠️ ANTHROPIC_API_KEY 未設定")
        print("  若測試失敗，請設定環境變數: export ANTHROPIC_API_KEY=your-key")
    ...
    # Test: Copilot SDK → ACP Proxy → claude-code-acp
    print("\n[TEST] Copilot SDK → ACP Proxy → claude-code-acp")
    ...
    try:
        ...
        session = await asyncio.wait_for(
            client.create_session({"model": "claude-sonnet-4-20250514"}),
            timeout=30.0
        )
        print("  Session 建立成功 ✅")
        ...
        t2 = time.time()
        await session.send({"prompt": "Say 'Hello from Claude via Proxy!' in one short sentence."})
    ...
    if all_pass:
        print("🎉 測試通過!")
        print("\n結論:")
        print("  Copilot SDK 可以透過 ACP Proxy 連接到 claude-code-acp!")
        print("  架構: Copilot SDK → copilot-acp-proxy → claude-code-acp")
    else:
        print("⚠️ 測試失敗")
